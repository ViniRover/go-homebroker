...
func NewBook(incomingOrder chan *Order, processedOrders chan *Order, wg *sync.WaitGroup) *Book {
	return &Book{
		Orders:          []*Order{},
		Transactions:    []*Transaction{},
...
func (b *Book) pricesMatch(order *Order, matchOrder *Order) bool {
	if order.OrderType == "BUY" {
...
func (b *Book) createTransaction(incomingOrder *Order, matchedOrder *Order) *Transaction {
	...
	if incomingOrder.OrderType == "BUY" {
		...
		sellOrder, buyOrder = incomingOrder, matchedOrder
	}
